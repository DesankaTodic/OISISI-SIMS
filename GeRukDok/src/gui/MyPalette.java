...
public class MyPalette extends JToolBar{
	...
	public MyPalette() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Object parent = path.getPathComponent(path.getPathCount()-2);
				 if (o instanceof GraphicSlot){
						...
						{	
							if (tabbDoc.getTitleAt(j) == d.getName())	//i poredi nazive(titlove) tabova sa nazivom dokumenta
							{
					    	...
					    	if(pg.getName() == pagePanel.getName()){
					    		for(int slotInd = 0; slotInd < pagePanel.getComponentCount(); slotInd++){
					    			if(pagePanel.getComponent(slotInd) instanceof GraphicSlotView){
						    			...
						    			if (s.getName() == slotPanel.getName()){
		...
		add(MyFrame.getInstance().getActionManager().getUndoAction());
		add(MyFrame.getInstance().getActionManager().getRedoAction());
		add(MyFrame.getInstance().getActionManager().getDeleteElementAction());
	
	}
	 public void enableRedoButton(boolean enable) {
		 MyFrame.getInstance().getActionManager().getRedoAction().setEnabled(enable);
	   }
	 public void enableUndoButton(boolean enable) {
		 MyFrame.getInstance().getActionManager().getUndoAction().setEnabled(enable);
	   }
	 ///////////////////////////////////
